...
import com.example.bank.Dialogs.SelectItemDialog;
import com.example.bank.Models.Item;
    ...
    private void initAdd() {
        ...
        if(!edtTxtItemPrice.getText().toString().equals("")){
            ...
            {//todo: execute
                ...
                txtWarning.setText("please set the date"); }

        }else{txtWarning.setVisibility(View.VISIBLE);
            txtWarning.setText("please as a price");}
    ...
    private class AddShopping extends AsyncTask<Void, Void, Void>{
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                transactionValue.put("used_id", loggedInUser.get_id());
                ...
                shoppingValues.put("amount", price);
                shoppingValues.put("description", description);
                shoppingValues.put("used_id", loggedInUser.get_id());
                shoppingValues.put("type", "shopping");
                shoppingValues.put("date", date);
                shoppingValues.put("recipient", store);
                db.insert("shopping", null,shoppingValues);

                Cursor cursor = db.query("users", new String[]{"remained_amount"}, "id=?", new String[] {String.valueOf(loggedInUser.get_id())},
                        null, null,null);
                if(null != cursor)
                {
                    if(cursor.moveToFirst()){
                        ...
                        Log.d(TAG, "doInBackground: affectetd rows: "+ affectedRows);
                    }
